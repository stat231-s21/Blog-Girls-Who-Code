...
First, we create scatter plots to explore the relationship between cluster assignment and our demographic variables of interest. 

```{r}
post_cluster_data <- read.csv("~/Desktop/DataScience/git/Blog-Girls-Who-Code/post_cluster_data.csv")

cluster_assignments <- kmeans_data2 %>% 
  select(SU_ID, clust4)

post_cluster_data <- post_cluster_data %>% 
  left_join(cluster_assignments, by = "SU_ID")

ggplot(data = post_cluster_data) 
  geom_mosaic(aes(x = GENDER2))
