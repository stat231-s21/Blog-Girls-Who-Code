...
In order to expand our Shiny app analysis, we wanted to explore more holistic relationships in the data by making complex comparisons between multiple variables at once. First, we used k-means clustering to form groupings in the data based on similarities in a few variables. We then continued our analysis by determining whether these clusters could predict the values of other variables. Because the clusters were designed to group respondents by the degree to which they had felt particular impacts of the pandemic, we explored the relationship between cluster assignment and both demographic and economic health variables. This allowed us to further explore which variables in the dataset best predict impacts of COVID-19 on individual economic health.

# Clustering Analysis

## K-Means Clustering

One of the first steps we took was determining which variables would be best to use to form our clusters. We had two primary requirements in mind: (a) variables have to be numeric and ordinal, and (b) variables should have enough variation to form clusters that are sufficiently different from each other. Because our data was based on a survey, almost all the variables were categorical. We chose variables that could be coerced into a numeric format, either because the question required the respondent to rate themselves on a scale or because the question had a set of other ordered, categorical answers. The variables we ended up including in our clustering analysis were:
...
As we can see, there is not a clear elbow in the plot for our data. While the decrease in inter-cluster variation is not constant, this decrease changes fairly steadily. We decided to perform the rest of our analysis using four clusters for two reasons:

1. The change in inter-cluster variation seems to decrease more slowly after the use of four clusters.
2. The centroids of our four cluster k-means analysis are more varied than the centroids of our k-means analyses with larger numbers of clusters.

<<<<<<< HEAD
We then created a pairs diagram to examine the differences between the clusters. Unfortunately, it’s difficult to determine a clear pattern in the scatter and density plots. 
=======
## Pairs Diagram
>>>>>>> a22b74997139b2a961f0fcf6930470f1309783f2

We then created a pairs diagram to examine differences between the clusters. Unfortunately, it is difficult to determine a clear pattern in the scatter and density plots. In order to better understand the variables driving cluster formation, we created separate density plots to simplify the pairs diagram.
...
While the original pairs diagram did not contain a lot of helpful information about the clusters, it was clear that income and age were driving variables in the cluster assignment. The variation between clusters was most clear in the density plots for these variables. 

## Density Plots

In order to further explore the patterns observed from our pairs diagram, we created the following density plots. These plots show the distribution of responses to the age and income questions across clusters.
...
ggplot(data = alL_vars_data, aes(x=ECON4A, group = clust4, fill=clust4)) +
  ...
  labs(title = "Distribution of 30-day Employment Confidence by Cluster", 
       x = "30-day employment confidence", 
       y = "Density",
       fill = "Cluster Assignment") +
  facet_wrap(~clust4)
...
ggplot(data = alL_vars_data, aes(x=ECON4B, group = clust4, fill=clust4)) +
  geom_density(adjust = 5) +
  labs(title = "Distribution of Three-Month Employment Confidence Answers by Cluster", 
       x = "Three month employment confidence", 
       y = "Density",
       fill = "Cluster Assignment") +
  facet_wrap(~clust4)
```
...
<<<<<<< HEAD
In the density plot comparing cluster assignment and both of the employment likelihood variables, it’s clear that clusters one and three are quite similar, as are clusters two and four. Clusters two and four are less confident as whole about their future employment, whereas clusters one and three are more confident. One notable difference between clusters one and three, in the graph for 30 day employment confidence, is that cluster three is more spread out across the variable, whereas cluster one contains more confident respondents.
=======
In the density plots comparing cluster assignment and both of the employment likelihood variables, it is clear that clusters one and three are quite similar, as are clusters two and four. Clusters two and four are less confident as whole about their future employment, whereas clusters one and three are less confident. In the graph for 30 day employment confidence, one notable difference between clusters one and three is that cluster three is more spread out across the variable, whereas cluster one contains more confident respondents.
>>>>>>> a22b74997139b2a961f0fcf6930470f1309783f2

# Post-Clustering Analysis

After performing our clustering analysis, we explored the relationship between different demographic and economic health variables and cluster assignment. The goal of this post-clustering analysis was to determine if cluster assignment could predict responses to a number of different demographic and economic health questions included in the COVID-Impact survey. 

## Demographic Variables

First, we examined the proportions of categorical demographic variable responses across clusters. The following demographic variables were used in this investigation:

* Gender: Male or Female
* Racial Background: Black (non-Hispanic), Hispanic, Other (non-Hispanic), White (non-Hispanic)
* Population Density: Rural, Suburban, Urban

Shown in the plots below, the proportions of both gender and population density responses did not differ substantially across clusters. In other words, all four clusters were made up of a similar proportion of male and female respondents; approximately 40% of respondents were male and approximately 60% of respondents were female across all clusters. Additionally, all four clusters were made up of a similar proportion of respondents from rural, suburban, and urban environments; approximately 75% of respondents lived in an urban environment, approximately 20% of respondents lived in a suburban environment, and approximately 5% of respondents lived in a rural environment across all clusters. Therefore, these visualizations demonstrate that cluster assignment does not predict responses to gender and population density questions well.
...
* TANF (Temporary Assistance for Needy Families) 
  - Question: In the past 7 days, have you either received, applied for, or tried to apply for any of the following forms of income or assistance, or not?
* Food Security
  - Question: Please indicate whether the following statement was often true, sometimes true, or never true for you or your household over the past 30 days, “The food that we bought just didn't last, and we didn't have money to get more.”
  
The plot below shows the proportion of responses to the TANF status question across clusters.
...
A majority of respondents reported that they did not apply for TANF across all four clusters. As a result, it is hard to visualize the proportions of respondents who may have applied for, tried to apply for, or received TANF in the plot above. For this reason, we also created the table below to analyze differences in proportions of responses to the TANF question. As over 97% of participants reported that they did not apply for TANF across all four clusters, cluster assignment does not predict very interesting or relevant variation in responses to the TANF status question. However, this table shows that respondents in clusters one and three reported that they tried to apply for or received TANF at a higher rate than respondents in clusters two and four.

```{r, include=FALSE}
TANF_data <- mosaic::tally(ECON6C_2 ~ clust4, data = alL_vars_data, format = "percent") %>% 
  as.data.frame() %>% 
  pivot_wider(names_from = ECON6C_2, values_from = Freq)
```

```{r}
knitr::kable(TANF_data, col.names = c("Cluster Assignmnet", 
                                      "Applied", 
                                      "Did Not Apply", 
                                      "Recieved", 
                                      "Tried to Appy")) %>% 
  kableExtra::kable_styling(position = "center", row_label_position = "c", 
                            latex_options = "hold_position")
```

The next visualization, presented below, demonstrates how the proportion of responses to the unemployment insurance questions change across clusters. The blue bars are larger in clusters one and three than in clusters two and four. This finding indicates that clusters one and three received unemployment insurance at a higher rate than respondents in clusters two and four. The purple bar is also largest in cluster three, indicating that respondents in cluster three tried to apply for unemployment insurance at a higher rate than respondents in the other clusters. However, as approximately 90% of respondents in each cluster did not apply for unemployment insurance, cluster assignment does not predict very interesting or relevant patterns of responses to the unemployment insurance status question.
