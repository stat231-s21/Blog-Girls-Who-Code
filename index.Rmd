...
A majority of respondents reported that they did not apply for TANF across all four clusters. As a result, it is hard to visualize the proportions of respondents who may have applied for, tried to apply for, or received TANF. For this reason, instead of creating a plot, we created the table below to analyze differences in proportions of responses to the TANF question.

```{r, echo=FALSE}
TANF_data <- alL_vars_data %>% 
  filter(ECON6C_2 != "Did not apply") %>% 
  select(clust4, ECON6C_2) %>% 
  group_by(clust4) %>% 
  summarise(did_not_apply = prop.table("Did not apply", margin = ECON6C_2))
```

```{r, results='asis'}
knitr::kable(TANF_data, "latex", booktabs = TRUE, escape = FALSE, 
             col.names = 
               c("Cluster Assignmnet", "Class Code", "Class Type", "Hours Spent on HW", 
                 "Hours Spent in Class", "Total Hours"))%>% 
